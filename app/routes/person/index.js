...
export default class PersonIndexRoute extends ClubhouseRoute {
  async model() {
    ...
    data.personMembership = (await this.ajax.request(`person/${personId}/membership`)).membership;
    data.personBanners = await this.store.query('person-banner', {person_id: personId, active: 1});
    data.photo = (await this.ajax.request(`person/${personId}/photo`)).photo;
    data.roles = (await this.ajax.request('role')).role;
    data.teams = (await this.ajax.request('team', {data: {can_manage: 1}})).team;
