...
export default class TrainingTrainersReportComponent extends Component {
  @argument students;
  @argument trainers;
  @argument training;
  @argument slot;
  ...
  didReceiveAttrs() {
    ...
    this.students.forEach((student) => {
      ...
      student.position_ids.forEach((id) => {
        if (this.TITLE_MAP[id]) {
          studentPositions[this.TITLE_MAP[id]] = 1;
        }
      })
    ...
    this.set('studentGroups', [
      { groupName: 'Veterans', students: vetStudents },
      { groupName: (this.training.is_art ? 'ART Alphas' : 'Binaries'), students: binaryStudents },
    ]);
  }
