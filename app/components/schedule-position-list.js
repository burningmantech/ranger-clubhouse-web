...
import {ADMIN, ART_TRAINER, ART_INTERFACE_BASE} from "clubhouse/constants/roles";
...
export default class SchedulePositionListComponent extends Component {
  ...
  constructor() {
    ...
    if (this.session.hasRole(ADMIN)) {
      ...
      this.canForceSignUp = true;
    } else if (this.isTrainingType && (this.session.hasRole(ART_TRAINER) || this.session.hasRole(ART_INTERFACE_BASE | position_id))) {
